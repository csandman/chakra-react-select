...
//   https://github.com/JedWatson/react-select/blob/react-select%405.10.2/packages/react-select/src/__tests__/StateManaged.test.tsx
...
describe("Select (state-managed, ported from react-select)", () => {
  const toggleMenuOpen = (container: HTMLElement) => {
    fireEvent.mouseDown(
  ...
  test("multi select > selecting multiple values", () => {
    const { container } = render(<Select {...BASIC_PROPS} isMulti />);
    openMenu(container);
  ...
  test("value prop > should not update the value on selecting option", async () => {
    ...
    await userEvent.click(
      container.querySelectorAll("div.react-select__option")[1]
    );
    expect(
      container.querySelector<HTMLInputElement>('input[type="hidden"]')!.value
    ).toBe("zero");
  ...
  cases(
    ...
    ({
      ...
      const toSelectOption = getByText(selectOption.label);
  ...
  interface KeyboardInteractionOpts {
    ...
    readonly eventsToSimulate: readonly [EventType, Record<string, unknown>][];
  ...
  cases<KeyboardInteractionOpts>(
    ...
    ({
      ...
      const { container } = render(<Select {...props} />);
      openMenu(container);
      for (const [eventName, eventArgs] of eventsToSimulate) {
        fireEvent[eventName](
          container.querySelector(".react-select__menu")!,
          eventArgs
        );
  ...
  test("`required` prop > should validate", async () => {
    ...
    // Indexed access is clearer than `const [, , , selectOption] = ...`
    // for the 4th element.
    // oxlint-disable-next-line prefer-destructuring
    const selectOption = container.querySelectorAll(
