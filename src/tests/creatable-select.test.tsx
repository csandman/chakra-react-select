...
describe("CreatableSelect (ported from react-select)", () => {
  ...
  cases<Opts>(
    ...
    ({ props }) => {
      ...
      ).not.toStrictEqual(expect.stringContaining("create"));
    },
    {
  ...
  cases<Opts>(
    ...
    ({ props }) => {
      props = { ...BASIC_PROPS, ...props };
      const filterOptionSpy = vi.fn<() => null>().mockReturnValue(null);
      ...
      expect(
        container.querySelector(".react-select__menu-notice--no-options")!
          .textContent
      ).toStrictEqual(expect.stringContaining("No options"));
    },
    {
      'single select > should not show "create..." prompt"': {},
      'multi select > should not show "create..." prompt"': {
  ...
  cases<Opts>(
    ...
    ({ props }) => {
      props = { ...BASIC_PROPS, ...props };
      const isValidNewOption = vi.fn((options) => options === "new Option");
      ...
      expect(
        container.querySelector(".react-select__menu")!.textContent
      ).toStrictEqual('Create "new Option"');
      ...
      expect(
        container.querySelector(".react-select__menu")!.textContent
      ).not.toStrictEqual('Create "invalid new Option"');
  ...
  cases<Opts>(
    ...
    ({ props }) => {
      ...
      expect(container.querySelector("input")!.textContent).toStrictEqual("");
    },
    {
  ...
  test("should remove the new option after closing on blur", () => {
    ...
    rerender(
      <Creatable menuIsOpen options={OPTIONS} inputValue="new Option" />
    );
  ...
  cases<Opts>(
    ...
    ({ props }) => {
      props = { ...BASIC_PROPS, ...props };
      const getNewOptionDataSpy = vi.fn((label) => ({
      ...
      expect(
        container.querySelector(".react-select__menu")!.textContent
      ).toStrictEqual("custom text new Option");
    },
    {
  ...
  cases<Opts>(
    ...
    ({ props = { options: OPTIONS } }) => {
      props = { ...BASIC_PROPS, ...props };
      const formatCreateLabelSpy = vi.fn((label) => `custom label "${label}"`);
      const { container, rerender } = render(
        <Creatable
          menuIsOpen
      ...
      expect(
        container.querySelector(".react-select__menu")!.textContent
      ).toStrictEqual('custom label "new Option"');
    },
    {
